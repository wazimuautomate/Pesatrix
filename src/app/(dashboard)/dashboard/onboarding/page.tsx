...
export default function DashboardOnboardingPage() {
  ...
  const [profileData, setProfileData] = useState<{ fullName: string; county: string } | null>(null);
  const {
    register,
    handleSubmit,
  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      try {
        ...
        if (payload.profile?.fullName || payload.profile?.county) {
          setProfileData({
            fullName: payload.profile.fullName ?? "",
            county: payload.profile.county ?? "",
          });
        }

        if (payload.status?.setupComplete) {
          router.replace("/dashboard");
          return;
        }
      } catch {
        // Allow manual completion if bootstrap fails.
      } finally {
        if (active) {
  ...
  async function onSubmit(values: OnboardingForm) {
    try {
      const hasName = Boolean(profileData?.fullName?.trim());
      const hasCounty = Boolean(profileData?.county?.trim());
      ...
      const response = await fetch("/api/onboarding/complete", {
        ...
        body: JSON.stringify({
          ...values,
          ...(hasName ? { fullName: profileData!.fullName } : {}),
          ...(hasCounty ? { county: profileData!.county } : {}),
        }),
      });
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <Card className="border-outline-variant/40">
        ...
        <CardContent>
          {isBootstrapping ? (
            <div className="flex min-h-40 items-center justify-center rounded-md border border-dashed border-outline-variant/40">
